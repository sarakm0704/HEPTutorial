void cut_ex2(string inputfile){
  ...
  TH1F * h_jet_multi = new TH1F("h_jet_multi","h_jet_multi",5,5,10);
  ...
  TH1F * h_cutflow = new TH1F("h_cutflow","h_cutflow",2,0,2);
  ...
  h_cutflow->SetBinContent(1,prenevents);
  h_cutflow->SetBinContent(2,nevents);
//  cout << " num. of sig. / sqrt(num. of. back.) " << << endl;

  TFile * f_out = new TFile(Form("out/cut/hist_%s",inputfile.c_str()),"RECREATE");
  h_cutflow->Write();
  h_muon_iso->Write();
